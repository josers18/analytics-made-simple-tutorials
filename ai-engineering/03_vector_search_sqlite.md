...
Four documents sit in a SQLite table. Each vector is three hand-written numbers, so the example runs without an embedding API. The query vector is closest to the SQL title. A production search would store vectors from an embedding model in that same `vector` column.
...
```bash
python3 03_vector_search_sqlite.py
```
...
| Rank | Title | Why it leads |
|---|---|---|
| 1 | SQL Joins and Entity Relationships | Nearest to the query vector |
| 2 | Pandas DataFrame Aggregations | Second nearest |
| 3 | Churn Prevention with Machine Learning | Far from the query |
| 4 | Customer Retention Rate Analysis | Farthest of the four |
