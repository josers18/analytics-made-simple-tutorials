# Customer triage with the TypeSafe SDK

> **Official companion for [Jev and TypeSafe AI](https://analyticsmadesimple.com/tutorials/jev-typesafe-ai-system-one-model-rlcd-tutorial/)**
> Raw script: [`01_customer_triage_engine.py`](./01_customer_triage_engine.py)

This is the same ticket as the Jev tutorial. The SDK call is `client.system_one`. Read `response.choices`, `response.scores`, and `response.nouls`. A live run prints a choice, a score, and a probability. Those numbers change from call to call.
...
python3 01_customer_triage_engine.py
```
...
[Payment routing](./02_sentiment_and_routing_pipeline.md)
